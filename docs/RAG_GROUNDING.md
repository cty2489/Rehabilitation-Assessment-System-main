...
6. Assist 应把知识 ID 绑定到实际采用该证据的句末；代码只承认逐句绑定的 ID，自动丢弃未绑定的顶层声明，并拒绝白名单外知识或由模型自行编造的数字引用。
...
9. 报告程序把内部 `[KB-*]` 映射为全文统一的 `【1】【2】`，同一文献只编号一次；JSON、网页、Word 和 PDF 同步保存逐句编号、知识条目、审核状态、来源 ID 和文末参考文献。
