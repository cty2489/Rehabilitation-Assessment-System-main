...
本阶段把 BGE-M3 + Qdrant 检索封装为只监听本机的独立服务，并为报告链路增加受控接入。目标不是立刻让知识库改变临床报告，而是先获得可复现的检索轨迹，验证命中质量、延迟和稳定性。

## 当前状态

- 知识源：`康复知识条目结构化整理.docx`。
- 解析结果：9 个条目，其中 7 个 `demo_ready`、2 个排除。
- 临床审核：当前 0 个 `clinical_ready`，因此不得用于正式报告生成。
- 语义检索：21 个改写问题实测 `Hit@1=0.8571`、`Hit@3=1.0`、`MRR=0.9286`。
- 已知缺口：尚未加入 reranker、证据充分性判断和知识库无答案拒答器。
- 云端验收：`cloud-server-v1.1.17` 已完成真实数据包 Shadow 全链路验证，命中证据未进入报告提示词。

因此，当前云端只允许部署 `shadow`。`assist` 必须保持审批关闭。
...
flowchart LR
    ...
    B --> C["BGE-M3 向量化"]
    C --> D["Qdrant 本地索引"]
    E["去标识化评估查询"] --> F["本机 RAG 服务 127.0.0.1:8010"]
    D --> F
    F --> G["证据包与来源元数据"]
    G --> H{"后端 RAG 模式"}
    H -->|off| I["不检索"]
    H -->|shadow| J["只写检索轨迹，不改报告"]
    H -->|assist| K["通过双重门禁后加入提示词"]
```

RAG 服务和报告后端使用不同 Python 环境。BGE-M3 固定在 CPU 上运行，避免占用报告大模型的 GPU 显存；Qdrant Local 同一时刻只由一个 RAG 服务进程打开。
...
| `assist` | 是 | 满足门禁时才会 | 未来经临床审核后的受控知识增强 |
...
6. Assist 报告会由代码确定性附加“辅助知识证据来源”表，不依赖大模型自行编造参考文献。
...
当前版本完成的是可部署的安全 Shadow 基座，不代表 RAG 已具备临床决策资格。
