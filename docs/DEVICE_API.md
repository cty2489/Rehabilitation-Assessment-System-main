...
新患者第一次评估前，设备端必须先把患者基本资料注册到云端。`patient_id` 由
设备端按 `DEV001_0001` 规则生成，云端将其作为全局业务主键；患者换到其他设备
评估时继续使用同一个编号。注册请求使用 JSON，不要把姓名、诊断等资料放在 URL。

```http
POST /api/device/v1/patients
Authorization: Bearer <DEVICE_API_TOKEN>
Content-Type: application/json
Idempotency-Key: DEV001:patient:DEV001_0001
```

请求体必须符合 [`device-patient-v1.schema.json`](schemas/device-patient-v1.schema.json)：
...
云端提供兼容开关 `DEVICE_REQUIRE_REGISTERED_PATIENT`。设备程序完成升级前保持 `0`；
联调通过后改为 `1` 并重启后端，届时未知患者上传评估会返回
HTTP 404 `PATIENT_NOT_FOUND`，评估阶段只使用云端档案。
...
| `patient_id` | 否 | 建议写入 `manifest.json`；若表单也提供，两处必须完全一致 |

旧版姓名、性别、年龄、诊断、患侧和病程表单字段仅在兼容期开关为 `0` 时读取。
正式对接不要再发送这些字段，云端统一根据已注册的 `patient_id` 读取档案。
...
### 4.2 application/zip raw 上传

如果设备端选择 `application/zip`，请求体就是 zip 文件本身，不再有 `package`
这个表单字段。元数据放在 query 参数或 `X-*` 请求头中。

```http
POST /api/device/v1/assessments/raw?patient_id=DEV001_0001
...
```bash
curl -X POST \
  "https://<cloud-host>/api/device/v1/assessments/raw?patient_id=DEV001_0001" \
  -H "Authorization: Bearer ${DEVICE_API_TOKEN}" \
  -H "Idempotency-Key: device_001:EVAL_20260629_001" \
...
1. 先注册一个测试患者，并重复提交一次验证 `created=false`。
2. 用已验证的设备端测试 zip 上传该患者的一次评估。
3. 每 5-10 秒轮询一次状态，不建议高频轮询。
4. 首版只下载 `export.zip`，设备端从中读取 `result.json` 并保存 `report.pdf`。
5. 下载后校验 `manifest.json` 中的 sha256。
6. 成功保存后再 ACK。
7. 在本地持久化 `job_id`；设备程序重启后继续轮询原任务。
8. 上传超时但未拿到响应时，使用相同 `Idempotency-Key` 重传。
...
| 404 | 严格模式下 `patient_id` 尚未注册，返回 `PATIENT_NOT_FOUND` |
| 409 | 患者编号身份冲突、请求与 manifest 患者编号不一致、幂等键冲突或任务尚未完成 |
