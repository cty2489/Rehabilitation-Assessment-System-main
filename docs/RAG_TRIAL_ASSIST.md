...
报告后端的 `backend/.env` 还必须指向同一份不可变发布包：
...
- `/v1/retrieve` 使用 BGE-M3 向量检索，为综合解读寻找相关辅助证据。
...
4. 大模型返回的引用全部属于本次证据白名单；只有逐条绑定到正文句末的 ID 才会进入参考文献，未绑定的顶层声明会被丢弃，编造 ID 或自行生成数字编号会触发重试或保守回退。
5. 最终 Markdown 使用 `【1】【2】` 形式，并在报告末尾列出知识审计表和去重参考文献，同时明确显示“内部技术验证”和“未完成正式专家审核”。
