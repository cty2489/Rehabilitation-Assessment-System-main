## test-snapshot-2026-07-24 - 当前测试版本说明

### 测试流程与展示

- `planner_rag` 报告链路将结构化观察、核心知识、检索计划、批量检索、报告生成和校验分工；数值与来源编号由确定性代码装配。
- 报告新增“综合亚型界定”：依据 Brunnstrom 手功能与 FMA 模型预测生成明确标记为测试性归纳的总括标签；历史测试结果由前端兼容补全，不改写数据库原文。
- 指标表改为 `指标｜本次结果｜解读｜依据` 四列。26 项指标的解读先说明测量目的，再说明本次数值是模型预测/设备计算值、可否与文献范围比较及为什么需要同条件复测。

### 知识库测试

- 知识库测试页返回来源类型、证据层级、权重、原始 HTTPS 链接、访问/版权说明和系统内短摘录。
- 证据层级优先展示公开指南与共识，教材只保留书目和章节定位，论文作为补充；不复制受版权保护全文。
- 当前功能用于测试和联调；专家审核为后续治理工作，不阻塞测试展示。

### 运行环境与验证

- 本地报告模型增加 CUDA 可用性检查，避免错误运行在 CPU 环境。
- 针对性后端测试、Python 编译、前端构建、目标 diff 检查和后端健康检查均已执行。详见 [测试快照 v2026.07.24](docs/TEST_SNAPSHOT_2026-07-24.md)。

# Changelog

## cloud-server-v1.1.24 - 2026-07-19

- 新增设备患者预注册接口 `POST /api/device/v1/patients`，使用设备 Bearer token 和 `rehab.patient.v1` JSON 请求；设备端生成的 `DEV001_0001` 形式编号作为 MySQL 全局患者业务主键。
- 注册以 `patient_id` 天然幂等：首次创建返回 HTTP 201，网络重试返回 HTTP 200 且不改写已有临床资料；同一编号对应不同姓名、性别或年龄时返回 HTTP 409，避免误覆盖患者档案。
- 评估上传会校验请求与 `manifest.json` 中的患者编号一致。新增 `DEVICE_REQUIRE_REGISTERED_PATIENT` 迁移开关；严格模式只读取云端患者档案，拒绝未知编号和不完整档案，兼容模式继续支持旧设备随评估上传资料。
- 新增患者注册 JSON Schema、设备端 Python/curl 示例、错误码和分阶段上线说明，并补充数据库事务与患者解析策略的自动化测试。

后端完整回归共 138 项测试通过；云端部署继续先使用兼容模式，待设备组完成注册接口联调后再启用强制预注册。

## cloud-server-v1.1.23 - 2026-07-18

- 重构前端为统一的临床工作台视觉系统：使用克制的中性色背景、深绿侧栏、青绿色主操作和蓝/绿/琥珀状态语义，移除页面中的可见渐变与装饰性背景。
- 统一登录页、顶部栏、主导航、页面标题、卡片、表格、表单、按钮、状态标签和报告区域的层级、间距、边框与交互反馈；引入 Lucide 图标替换字符图标和不一致的符号。
- 调整侧栏信息架构和业务命名，将评估记录、设备接口、RAG 知识库及平台配置清晰分组，并为模型设置和系统管理保留独立入口。
- 优化仪表盘、患者管理、康复评估、统计分析、模型设置、设备接口和系统管理等高频页面；删除登录页与评估页中过时的固定模型宣传文字。
- 完成 1280×720 桌面端与 390×844 窄屏黑盒回归；窄屏侧栏自动收窄为图标导航，表单改单列，知识库标签改为稳定两列，页面无横向溢出或控制台警告。

前端 TypeScript 与 Vite 生产构建通过，运行依赖审计为 0 个漏洞；桌面端及移动端主要业务页面完成浏览器截图检查。

## cloud-server-v1.1.22 - 2026-07-18

- 将管理员侧“知识与证据治理”重构为面向业务展示的“RAG知识库”，页面主线统一为知识内容、26 项指标映射、参考文献和报告引用。
- 概览新增真实报告检索链路：评估上下文、自动构造查询、总体知识向量召回、26 项 `system_key` 精确匹配，以及正文 `【n】` 数字引用。
- 摘要区只显示知识条目数、26 项映射数、参考文献数、检索服务状态和报告接入状态；移除阻断、临床可用、专家确认等内部治理指标。
- 知识表格移除治理状态和算法检查字段；详情只展示知识摘要、报告解读参考、数值解释说明、适用范围和参考文献，并隐藏标题中的研发备注。
- 后端知识审核状态、Assist 门禁、发布包校验和算法检查保持不变，仅从业务展示页面移除；README 与 RAG 试运行说明同步更新。

前端生产构建通过，并使用真实的 35 条知识、26/26 指标映射和 33 项结构化来源完成桌面端、移动端、知识列表及详情抽屉黑盒检查。

## cloud-server-v1.1.21 - 2026-07-18

- 报告新增论文式数字引用：大模型仍使用受控的内部 `[KB-*]` 绑定具体句子，确定性代码按正文首次出现顺序转换为 `【1】【2】`，同一文献全文只编号一次。
- 报告末尾新增“知识库证据审计”和“参考文献”：读者看到标准文献编号，管理员仍可追溯知识 ID、来源 ID、知识状态和审核状态；没有外部文献的条目会明确标记为“非外部文献”。
- 引用门禁拒绝白名单外知识和模型自行生成的数字编号；模型仅在顶层多报、但未绑定到具体句子的 ID 会被安全丢弃，不再造成无依据参考文献或整份报告回退。
- `result.json` 为总体解读、逐项 biomarker 解读/建议、综合亚型、治疗策略和预警增加对应引用编号，并在 `knowledge_evidence.references` 保存编号、来源 ID、标准引用文字和关联知识条目。
- 网页将 `【n】` 显示为带提示的上标；Word、JSON 和 PDF 共用同一编号目录，旧版证据表仍可兼容解析和导出。

云端使用 Qwen3-8B、试运行知识集合 `rehab_knowledge_trial_v0_2` 完成真实 Assist 验收：报告保持 `generation_mode=llm`，精确知识引用有效，正文无内部 KB token，生成 9 条去重参考文献；JSON 中 9 个编号与 3 个知识条目映射一致，PDF/Word 同步显示。后端全量 128 项测试、云端聚焦 67 项测试、前端生产构建和启动脚本语法均通过。试运行知识仍未完成正式专家审核，只允许内部技术验证。

## cloud-server-v1.1.20 - 2026-07-18

- 新增管理员只读“知识与证据治理”页面：分别显示知识内容版本、索引集合、应用版本和报告模型，并明确区分 `26/26` 系统指标映射与 `0/26` 临床可用状态。
- 新增知识状态、条目、26 项映射、条目详情和来源文献管理接口；全部沿用管理员会话鉴权，浏览器不直接访问 Qdrant 或本机 RAG 服务。
- 结构化审阅 JSON 入库新增 `sources.jsonl`，保留 33 项来源及其知识条目关系；准备阶段校验来源编号、URL 协议和主机，读取阶段再次校验 schema、集合、唯一键、数量及双向关联。
- 知识页面支持按模态、治理状态和关键词筛选，并通过证据抽屉集中显示允许解释、禁止解释、采集算法要求、参考范围政策、实施动作和原始来源。
- 系统管理中的平台版本改为读取后端 `APP_VERSION` 与 `APP_BUILD_COMMIT`，移除过期的前端硬编码版本。
