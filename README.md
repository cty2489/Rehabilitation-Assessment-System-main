...
当前云端还有一份在 `cloud-server-v1.1.24` 基础上继续迭代的测试快照：

- `planner_rag` 按固定顺序完成结构化观察、核心知识绑定、检索计划、一次批量检索、报告生成和校验；
- 新报告显示“综合亚型界定”，历史测试结果由前端兼容补全；
- 指标表采用 `指标｜本次结果｜解读｜依据` 四列，26 项指标先说明检测目的，再说明本次数值性质和复测边界；
- 知识库测试页按指南/共识、教材、论文的层级展示来源卡、原始链接和系统内短摘录；
- 本快照仅用于测试和联调，专家审核是后续治理工作，不阻塞当前测试功能。

完整流程、边界、验证和上传范围见 [测试快照 v2026.07.24](docs/TEST_SNAPSHOT_2026-07-24.md)。
